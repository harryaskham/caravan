...
Explicitly **not** included: automatic installation onboarding, provider
tenancy reconciliation, and automatic failover. Exactly one worker may be the
configured writer per repository; a second host may read and plan. Installation
lifecycle/tenancy remains separate work.
