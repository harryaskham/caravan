...
| terminal-red green recovery | `cara unpark --repository-slug OWNER/NAME --pr N --head H --base-ref BRANCH --base B --membership-generation G --parking-fingerprint F --provider-state open_parked --actor A --reason R` | envelope | `unpark` | exact parked provenance/provider/membership generation, newest authoritative green checks, explicit-pause exclusion, writer lease, post-mutation fleet rediscovery, durable idempotent receipt |
