...
| reviewed force intent | `cara force-intent preview/apply/revoke --pr N --head OID --membership-generation G --failure-fingerprint F --reason R --expires-at-ms T --auto-merge squash` | envelope | `force_intent_preview`, `force_intent_apply`, `force_intent_revoke` | fresh exact provider/membership/check/decision evidence, shared deterministic fingerprint, zero-write preview, single-request label+auto-merge apply, expiry-safe idempotent revoke, bounded partial receipts |
...
| synchronize | `cara sync [--all] [--rerun-failed]` | envelope | `sync` | idempotence, rolling head, exact root/tail/member scheduler generations, authenticated provider telemetry/budget, healthy/waiting/held no-wake status, retry-vs-external-decision wake class, opt-in bounded priority/FIFO greedy admission with generation-bound skips, safe affected-PR checkout, exact-run rerun, intentional hold skips |
| managed repair | `cara repair start/authorize-agent-edits/grant/revoke-grant/status/continue/abort` | envelope | `repair_start`, `repair_authorize_agent_edits`, `repair_grant`, `repair_revoke_grant`, `repair_status`, `repair_continue`, `repair_abort` | dirty caller isolation, exact-provider materialization, narrow conflicts/grants plus exact session-level agent authority, bounded path/diff fingerprints, fresh-CI/exact-lease publication, interruption-safe resume |
...
| terminal-red green recovery | `cara unpark --repository-slug OWNER/NAME --pr N --head H --base-ref BRANCH --base B --membership-generation G --parking-fingerprint F --provider-state open_parked --actor A --reason R` | envelope | `unpark` | exact parked provenance/provider/membership generation, protection-declared required-context success, check-sensitive preflight, explicit-pause exclusion, writer lease, post-mutation fleet/required-check rediscovery, provider-write-free refusal, durable idempotent receipt |
| native Stack ancestry recovery | `cara native-stack rebase-preview/rebase-apply --stack N --actor A --reason R [--expected-plan-hash H]` | envelope | `native_stack_rebase_preview`, `native_stack_rebase_apply` | exact adjacent ancestry evidence, complete divergent-suffix plan, isolated preparation, atomic multi-ref force-with-lease, provider-linear postcondition, fresh-CI requirement |
| paused-generation recovery | `cara pause-recovery prepare/checkpoint-base/checkpoint-head/finalize/rollback` | flat v1 data envelope | `pause_recovery_prepare`, `pause_recovery_checkpoint_base`, `pause_recovery_checkpoint_head`, `pause_recovery_finalize`, `pause_recovery_rollback` | exact owner/pause/topology binding, provider-write-free checkpoints, strict ordinary resume guard, one-commit and synthetic merge/check attestation, drift/concurrent-sync fence, crash-safe terminal replay, exact old-state rollback |
| foreground ticks | `cara loop [--once] [--manual --shell COMMAND]` | one-shot envelope only; manual refused | intentionally absent | canonical sync/hooks; failed ticks dispatch hooks and keep ticking with bounded evidence; CLI-only external-decision evidence file + inherited TTY shell + exact rediscovery |
