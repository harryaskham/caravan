...
- A valid partial repository survives interruption and is resumed in place, reusing already received objects. Reuse requires both exact provider `origin` and exact canonical `cache` remotes; pre-object-cache full-clone remnants cannot bypass seeding. Invalid partial state is removed only after canonical manifest/path validation.
...
- Object-cache extension landed at `94687c4`; post-land cache-binding guard is commit `513227f` before final consolidation. Final landed squash SHA will come from the reintegration receipt.
- Summary artefact commit: intentionally omitted; this file must not self-reference its own mutable SHA.
- Files touched: `src/repair.rs`, `README.md`, `SPEC.md`.
- Tests: 11 focused repair tests, including cache/provider remote separation under dirty/internal-origin caller, cache identity drift refusal, sideband classification, cache-bound valid partial-repository reuse, no-workspace resume, timeout group receipt, scope/head/parent/non-force/abort guards.
