...
fn failing_unjoined_candidate_is_skipped_after_existing_caravan_progress() {
    ...
    let merge_at = calls
        .iter()
        .rposition(|kind| *kind == MutationKind::SquashMerge)
        .expect("existing caravan merge occurs");
    let skip_at = calls
        .iter()
        .enumerate()
        .skip(merge_at + 1)
        .find_map(|(index, kind)| (*kind == MutationKind::Comment).then_some(index))
        .expect("candidate refusal receipt occurs after existing convergence");
    assert!(
        merge_at < skip_at,
...
fn exact_unjoined_admission_gate_defers_heavy_ci_without_hiding_other_failures() {
    ...
    let mut duplicate_identity = candidate.clone();
    duplicate_identity.checks.push(CheckSnapshot {
    ...
    let duplicate_status = status(
        vec![duplicate_identity.clone()],
        Some(duplicate_identity.number),
        &clean,
    );
    let duplicate_provider = FakeProvider::with_pull_requests(vec![duplicate_identity.clone()]);
    duplicate_provider.require_contexts("main", &[&gate.context, "heavy-ci"]);
    duplicate_provider.serve_lineage(
        duplicate_identity.number,
        HeadRunLineage {
            head_sha: duplicate_identity.head.oid.0.clone(),
    ...
    let mut duplicate_progress = SyncProgress::new(&duplicate_status, Vec::new(), u32::MAX);
    assert!(
        candidate_local_admission_refusal(
            &duplicate_provider,
            &mut duplicate_progress,
            &duplicate_status.repository,
            duplicate_identity.number,
            Some(&gate),
        )
...
fn a_foreign_auto_merge_request_is_converged_away_or_refused_but_never_raced() {
    ...
    assert_eq!(
        *provider.calls.borrow(),
        ...
        "the foreign merge actor is removed and the exact merge is audited before cara merges"
    );
    assert!(progress.steps.iter().any(|step| {
