...
pub fn snapshot(context: &AppContext, input: &LogInput) -> Result<LogOutput, AppError> {
    ...
    let paths = paths(&context.repository_path)?;
...
pub fn follow(
    ...
    while !stop.load(std::sync::atomic::Ordering::SeqCst) {
        ...
        for (key, count) in observed {
            seen.entry(key)
                .and_modify(|seen_count| *seen_count = (*seen_count).max(count))
                .or_insert(count);
        }
    }
    Ok(())
...
fn paths(repository: &Path) -> Result<JournalPaths, AppError> {
    let output = Command::new("git")
        .args(["rev-parse", "--path-format=absolute", "--git-common-dir"])
        .current_dir(repository)
        .output()
        .map_err(|error| io_error("journal_git_metadata_failed", &error))?;
    if !output.status.success() {
        return Err(AppError::structured(
            ErrorCategory::ExecutionFailure,
            "journal_git_metadata_failed",
            "could not resolve common Git metadata directory",
            Some(json!({ "exit_code": output.status.code() })),
        ));
    }
...
mod tests {
    ...
    fn torn_final_record_is_ignored_and_recovered_before_the_next_append() {
        let (_repository, context) = fixture();
        append_event(&context, &event(1)).unwrap();
        let journal = paths(&context.repository_path).unwrap();
        OpenOptions::new()
            .append(true)
            .open(&journal.active)
            .unwrap()
