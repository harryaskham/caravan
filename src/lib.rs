...
6. Run `cara sync` for the current caravan or `cara sync --all` for the fleet.
   ...
   or changing the caller's branch/index/worktree. Provider/ref commands retain
   the configured timeout; local detached-worktree creation has an independent
   60-second floor, 120-second cap, and whole-sync-budget ceiling. The fetched
