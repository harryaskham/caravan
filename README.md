...
- Optional sync-owned auto-admission is strictly opt-in. After the existing
  ...
  branch as the candidate's virtual tail. If GitHub leaves the synthetic first
  parent stale, or only the recorded PR base stale after the synthetic parents
  catch up, Cara may create the new root without rewriting its source head
  only after complete same-repository/two-parent identity, exact remote Git
  ...
  `exact_git_proof` stale-base recovery, retaining the stale observation even
  for projection-only recovery. Caller-reviewed base/default leases stay strict.
...
separate admission-only writer runs `cara admit`, rerequests the selected PR's
existing suite, and never runs heavy jobs for its own wake PR. Copy the reviewed
three-workflow bundle from [`examples/workflows/`](examples/workflows/):
`caravan-gate.yml`, `cara-admit.yml`, and `cara-sync.yml`. Replace the Cara
version and archive SHA-256 placeholders together in one reviewed change.
Admission and convergence use the byte-identical
