...
- `cara check` — no-update validation. `check --pr`, `new --pr` and `join --pr` accept the optional schema-one JSON `--expected-admission` caller lease documented in [caller-reviewed admission](docs/caller-reviewed-admission.md). The engine verifies repository/PR/source/base/default/unjoined facts in the read and writer paths before effects, keeps existing policy and write fences, preserves exact input values in CHECK data and nested `join_receipt` echoes, and never equates a post-effect refusal with zero effects or automatic replay. Guarded immutable admission requires exact predecessor ancestry; no-force intent uses the caller ABI value `none` (legacy `absent` remains readable). For an active member, check its whole caravan and fleet invariants. For an unenrolled candidate with no explicit target, evaluate attachment to the first active, unparked, unheld caravan in canonical ascending root-PR order. A clean attachment returns the complete coherent `join` receipt; an ineligible attachment falls back to the ordinary `new` evaluation. Targetless `cara join --pr N` selects that same first target and never searches later caravans after an incompatible preflight. Parked and held caravans remain exact repair evidence but are not implicit join targets; an explicit parked target returns `caravan_target_parked` with the head and responsible actor. An explicit `cara new` mutation continues to preflight `new`. No available target retains the independent `new` recommendation and makes targetless join return a typed refusal.
...
`join`/`rejoin` without a target deterministically select the first active, unparked, unheld caravan in canonical ascending root-PR order. They do not search later caravans after an incompatible first-target preflight. When no such target exists they fail with typed visible-caravan evidence; explicit `--tail-pr`/`--head-pr` selection remains unchanged.
...
blocks; repository-global problems with no parked owner fail closed. Hooks are
