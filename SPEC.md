...
- `cara check` — no-update validation. For an active member, check its whole caravan and fleet invariants. For an unenrolled candidate with no explicit target, first evaluate attachment to the one visible unheld, unparked caravan. A clean attachment returns the complete coherent `join` receipt; an ineligible attachment falls back to the ordinary `new` evaluation. Zero or multiple active caravans retain `new` evaluation because a subsequent targetless `cara join` could not resolve the same target unambiguously. Parked caravans remain exact repair evidence but are not join/new compatibility targets: an explicit parked target returns `caravan_target_parked` with the head and responsible actor. This recommendation policy is check-only: an explicit `cara new` mutation continues to preflight `new`.
