...
No local lock can serialize distributed machines. Every mutation therefore carries optimistic preconditions over PR number, head SHA, base ref/SHA, labels, state, and auto-merge state. CI/check progress is observation, not membership mutation identity: queued/running/completed churn alone cannot stale an exact join root. A real mismatch aborts with `stale_precondition`; the caller rediscover/reruns rather than overwriting concurrent work. Human CLI errors show bounded colored changed-field/count/OID/next-action summaries and direct operators to `--json` when full evidence exceeds 4 KiB; JSON/MCP always retain complete details.
