...
- `cara web --repo PATH...` — loopback path-scoped dashboard, typed actions/plans/progress/journal, optional authenticated GitHub webhook wake receiver, collapsible repository/attention sidebars, and a center Saloon for unenrolled PRs. Saloon groups are ordered Ready to Roll, Saddling Up, Other, Bounty List; fresh candidate evidence wins over stale skip labels so fixed unjoined PRs return to Ready. Accepted actions bind the reviewed refresh sequence plus a deterministic mutation-authority fingerprint over exact config/provider/topology facts. Refreshes coalesce behind active jobs; sequence-only drift with an identical fingerprint is harmless, while real drift fails before mutation with expected/actual fingerprints.
